...
//
  //               }
    ...
    public int checkProductIndex(String productId){    //to kanw opws ta epanw me equals gia ta string   //REPAIR OPWS TA EPANW
        ...
        ListIterator<Product> iter = shopList.get(0).getProductList().listIterator();
        int index = -1;

        while (iter.hasNext()) {
            index++;
            if (productId == iter.next().getProductId()) { //iter.next().getProductId()
                return index;
            }
        }
    ...
    public void dataNormaliserCSV(int numberOfColumns){
        ...
        String cvsSplitBy1 = ",";
            ...
            try {
                PrintWriter pw1 = new PrintWriter(new File("inputNormalised_NN1.csv"));
